...
data.forEach(user => {
  const phrase = `${user.company.catchPhrase} - ${user.company.bs}`;
  companyPhrases.push(phrase);
});
console.log(companyPhrases);


// query the HTML for the class class-week, then use forEach to change the text content

document.addEventListener("DOMContentLoaded", function () {
  const classWeeks = document.querySelectorAll(".class-week");

  classWeeks.forEach((week, index) => {
    console.log(`Week ${index + 1}:`, week.textContent.trim());
  });
});
